...
public class Client {
	...
	public static void main(String[] args) {
		...
		System.out.println("3 places for the park and 114514 places for chemin,feel free to fulfill the smelly chemin"+"\n"+"$10 for each hour,nice day!"+"\n");
		
		double price=10;
		
		System.out.println("a or A means arrive;enter plate and press the ENTER bottom;Then give the time of arrival,and press the ENTER bottom"
				+"\n"+ "d or D means departure;enter plate and press the ENTER bottom;then enter the time of leaving,enter plate and press the ENTER bottom"
		...
		if (instruction.equalsIgnoreCase("A")) {
			...
			if (park.isFull()) {
				...
				System.out.println(cnt+" cars waiting ahead in chemin and you are the "+(cnt+1)+" one You might continue");
				continue;
			}
				...
				System.out.println(cnt+" cars in the park before this car and this is the "+(cnt+1)+"ST/ND/RD/TH one You might continue");
				continue;
			}
			...
			if (park.isEmpty()) {System.out.println("Empty park!");continue;}
				...
				if (rd.isEmpty()!=true) {
					Voiture masque=rd.driveOut();
					masque.setStartTime(time);
					park.leadIn(masque);
